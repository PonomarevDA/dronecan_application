...
typedef struct {
    ...
    uint16_t
        command;  // Either a binary command (0 - release, 1+ - hold) or bitmask
} HardpointCommand_t;
...
static inline int8_t dronecan_equipment_hardpoint_command_serialize(
    const HardpointCommand_t* const obj, uint8_t* const buffer,
